...
public class AirportController {
    ...
    public AirportController(Storage storage) {
    ...
    public Response registerAirport(String id, String name, String city, String country, String latitudeS, String longitudeS){
        try{
            ...
            if(!validChars(id))  //De ninguna manera ID puede no tener minimo 3 letras mayusculas
                return new Response("Airport's ID must be exactly 3 uppercase letters", Status.BAD_REQUEST);
            ...
            try { //Rango de las latitudes (ojala ni uno se dañe)
                ...
                if (latitude < -90 || latitude > 90 || !maxDec(latitudeS))
                    return new Response("Latitude must be between -90 and 90 with up to 4 decimal places", Status.BAD_REQUEST);
            ...
            try { //Rango de las longitudes (y aqui tampoco)
                ...
                if (longitude < -180 || longitude > 180 || !maxDec(longitudeS))  //Rango de las longitudes
                    return new Response("Longitude must be between -180 and 180 with up to 4 decimal places", Status.BAD_REQUEST);
            } catch (NumberFormatException e) {
            ...
            return new Response("Airport registered successfully", Status.CREATED);
        }catch (Exception e){
